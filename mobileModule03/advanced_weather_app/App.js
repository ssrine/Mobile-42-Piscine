...
const searchCitiesWithFallback = async (query, count = 5) => {
  const variants = buildQueryVariants(query);
...
function WeatherApp() {
  ...
  const loadWeatherForPlace = async ({ place, query, nextSearchText }) => {
    ...
    try {
      ...
      if (!resolvedPlace) {
        ...
        try {
          matches = await searchCitiesWithFallback(trimmedQuery, 5);
        } catch {
  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(async () => {
      ...
      try {
        const matches = await searchCitiesWithFallback(query, 5);
